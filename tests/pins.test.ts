...
import { checkLocks } from '@mica/build-tools'
import { assertEnvironmentImage, buildEnvImages, environment, ids, sources } from '../src/pins.ts'
import { REPO, workdir } from './fixture.ts'

const work = workdir('pins')
...
// A mica-build-env release lock, as that repository publishes it. The upstream registry host is spelled in parts, so this file names no
...
// locks/ holding one build-env lock and its pin, read as mica-build-tools reads it.
function locks(name: string, lock: string): { inputs: ReturnType<typeof checkLocks>, dir: string } {
  const dir = join(work, name)
  mkdirSync(join(dir, 'pins'), { recursive: true })
  writeFileSync(join(dir, 'mica-build-env.lock'), lock)
  writeFileSync(join(dir, 'pins/mica-build-env.pin'), `# mica-pin v1\nREPOSITORY=mica-build-env\nRELEASE=20260914-2353\nSHA256SUMS=${'f'.repeat(64)}\n`)
  return { inputs: checkLocks(dir, 'local'), dir }
}

test('the build-env lock gives its base and C platform manifests and the upstream frontend and BuildKit', () => {
  const images = (name: string, lock: string): ReturnType<typeof buildEnvImages> => {
    const { inputs, dir } = locks(name, lock)
    return buildEnvImages(inputs, dir)
  }
  expect(images('good', LOCK)).toEqual({
  ...
  expect(() => images('no-c', without('mica-build-env', 'c', 'arm64'))).toThrow('mica-build-env:c@arm64')
  expect(() => images('no-frontend', without('upstream', 'docker/dockerfile:1-labs', 'arm64').replace(/^image\tupstream\tdocker\/dockerfile:1-labs\tamd64.*\n/m, ''))).toThrow('docker/dockerfile:1-labs')
  const second = LOCK.replace(/(image\tupstream\tmoby\/buildkit:v0\.33\.0\tarm64\t[^\n]+\n)/, `$1image\tupstream\tmoby/buildkit:v0.34.0\tamd64\t${BUILDKIT.replace('v0.33.0', 'v0.34.0')}\n`)
  expect(() => images('two-buildkits', second)).toThrow('one moby/buildkit:<tag>')
})

// A third-party image is taken only from a row of a lock under locks/: no other
// tracked file names an image by a registry reference
...
test('no tracked file outside locks/ names a third-party image', () => {
  const git = Bun.spawnSync(['git', '-c', 'safe.directory=*', '-C', REPO, 'grep', '-nIE', String.raw`(@sha256:[0-9a-f]{64}|docker\.io|^\s*FROM\s|^# syntax=)`, '--', '.', ':!locks'], { stdout: 'pipe' })
